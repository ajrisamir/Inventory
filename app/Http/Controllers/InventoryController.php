...
class InventoryController extends Controller
{
    ...
    public function exportInventory()
    {
        ...
        if ($user_type == 'suadmin') {
            ...
            return redirect(url('index'))->with("fail", "Only SuAdmin can Export Item");
        }
    }
    ...
    // Menambahkan fungsi untuk update stok
    public function updateStock(Request $request)
    {
        // Validasi input
        $request->validate([
            'item_name' => 'required|string',  // Nama produk yang akan diperbarui
            'item_stock' => 'required|integer|min:1',  // Jumlah yang akan ditambahkan
        ]);

        // Mencari produk berdasarkan nama
        $item = DB::table('item')->where('item_name', 'LIKE', '%' . $request->item_name . '%')->first();

        if ($item) {
            // Jika produk ditemukan, update stok
            DB::table('item')
                ->where('id', $item->id)  // Berdasarkan ID produk
                ->update([
                    'item_stock' => $item->item_stock + $request->item_stock,  // Menambahkan stok
                    'item_updated_at' => now(),  // Update timestamp
                ]);
            ...
            return response()->json([
                ...
                'updated_stock' => $item->item_stock + $request->item_stock,
            ]);
        } else {
